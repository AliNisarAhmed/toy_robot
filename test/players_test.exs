defmodule ToyRobot.Game.PlayersTest do
  ...
  describe "change position if occupied" do
    setup do
      table = %Table{
        north_boundary: 1,
        east_boundary: 1,
    ...
    test "changes position if it is already occupied", %{table: table} do
      occupied_positions = [%{north: 0, east: 0}]
      original_position = %{north: 0, east: 0, facing: :north}

      new_position = Players.change_position_if_occupied(
        occupied_positions, table, original_position
      )
