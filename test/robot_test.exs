defmodule ToyRobot.RobotTest do
  ...
  describe "when the robot is facing north, and it has moved east one" do
    setup do
      {:ok, %{robot: %Robot{east: 1, facing: :north}}}
    end

    test "turns north one space", %{robot: robot} do
      robot = robot |> Robot.move()
      assert robot.north == 1
      assert robot.east == 1
  ...
  describe "when the robot is facing south" do
    ...
    test "it moves one space to the south", %{robot: robot} do
      robot = robot |> Robot.move()
      assert robot.north == -1
    end
  ...
  describe "when the robot is facing south, and it has moved east one" do
    setup do
      {:ok, %{robot: %Robot{east: 1, facing: :south}}}
    end

    test "turns south one space", %{robot: robot} do
      robot = robot |> Robot.move()
      assert robot.north == -1
      assert robot.east == 1
      assert robot.facing == :south
